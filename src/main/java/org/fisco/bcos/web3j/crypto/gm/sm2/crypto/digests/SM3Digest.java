package org.fisco.bcos.web3j.crypto.gm.sm2.crypto.digests;

import org.fisco.bcos.web3j.crypto.gm.sm2.util.Memoable;
import org.fisco.bcos.web3j.crypto.gm.sm2.util.Pack;
...
public class SM3Digest extends GeneralDigest {
	...
	protected void processWord(byte[] in, int inOff) {
		...
		int n = (((in[inOff] & 0xff) << 24) | ((in[++inOff] & 0xff) << 16)
	...
	protected void processBlock() {
		...
		for (int j = 16; j < 68; ++j) {
			...
			this.W[j] = P1(this.W[j - 16] ^ this.W[j - 9] ^ r15) ^ r7
					^ this.W[j - 6];
		}
		for (int j = 0; j < 64; ++j) {
