...
public class JsonRpc2_0Rx {
  ...
  public Flowable<BcosBlock> blockFlowable(boolean fullTransactionObjects, long pollingInterval) {
    return blockHashFlowable(pollingInterval)
        .flatMap(blockHash -> web3j.getBlockByHash(blockHash, fullTransactionObjects).flowable());
  }
    ...
    if (ascending) {
      return Flowables.range(startBlockNumber, endBlockNumber)
          .flatMap(
              i ->
                  web3j
                      .getBlockByNumber(new DefaultBlockParameterNumber(i), fullTransactionObjects)
                      .flowable());
    } else {
  ...
  public Flowable<BcosBlock> replayPastAndFutureBlocksFlowable(
    ...
    return replayPastBlocksFlowable(
        startBlock, fullTransactionObjects, blockFlowable(fullTransactionObjects, pollingInterval));
  }
