...
public class QueuingTransactionReceiptProcessor extends TransactionReceiptProcessor {
  ...
  /**
   * Java doesn't provide a concurrent linked hash set, so we use a simple wrapper to store details
   * of the number of requests we've made against this specific transaction hash. This is so we can
   * preserve submission order as we interate over the outstanding transactions.
   *
