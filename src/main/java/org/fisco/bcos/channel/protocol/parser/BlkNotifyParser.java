...
public class BlkNotifyParser {
	...
	public BcosBlkNotify decode(byte[] data) throws JsonParseException, JsonMappingException, IOException {
		BcosBlkNotify bcosBlkNotify = new BcosBlkNotify();

		switch (getVersion()) {
		
		case VERSION_2: {
			bcosBlkNotify = ObjectMapperFactory.getObjectMapper().readValue(data, BcosBlkNotify.class);
		}
			break;
		case VERSION_1: {
			...
			if (split.length != 2) {
				throw new IllegalArgumentException(" invalid block notify message , data is " + data0);
			}
