...
public class EventLogFilterManager {
    ...
    public synchronized void addEventLogFilter(EventLogFilter filter) {
        registerIDToFilter.put(filter.getRegisterID(), filter);
        logger.info("add, registerID: {}, filter: {}", filter.getRegisterID(), filter);
    }

    // remove filter
    public synchronized void removeFilter(String registerID, String filterID) {
    ...
    // remove filter
    public synchronized void removeFilter(String registerID) {
        registerIDToFilter.remove(registerID);
        logger.info("remove event log filter, registerID: {}", registerID);
    }

    public synchronized void updateEventLogFilter(
